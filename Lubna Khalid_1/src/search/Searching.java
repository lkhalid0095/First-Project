...
public class Searching {
	...
	private void proceedWithSearch() {
		...
		int index = binarySearchAlgorithm(numbers ,0,numbers.length-1,target);
	...
	private int binarySearchAlgorithm(int[] searchThis, int startIndex, int endIndex, int target) {
		...
		if(searchThis[index] == target)
			...
			return binarySearchAlgorithm(searchThis ,startIndex,index-1,target);
		}
		else return binarySearchAlgorithm(searchThis ,index+1,endIndex,target);
		}
